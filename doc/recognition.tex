...
prime and $p^n \leq 255$ existed in {\GAP} until version 4.1.6. The following functions
...
This function returns the {\IRREDSOL} id (see "IdIrreducibleSolubleMatrixGroup") of <G>, 
where <G> is the group created by `IrreducibleSolubleGroupMS'(<n>, <p>, <k>) in {\GAP}, available in GAP until GAP 4.16.

\testexamplefalse
\beginexample
gap> IdIrreducibleSolubleMatrixGroupIndexMS(6, 2, 5);
[ 6, 2, 2, 4 ]
gap> G := IrreducibleSolubleGroupMS(6,2,5); # only available until GAP 4.16
...
`IrreducibleSolubleGroupMS'(<n>, <p>, <l>), available in GAP until GAP 4.16,  is conjugate to
...
[ 6, 2, 13 ]
gap> G := IrreducibleSolubleGroupMS(6,2,13); # only available until GAP 4.16
